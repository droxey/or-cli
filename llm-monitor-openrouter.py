"""Transparent LLM proxy: monitors OpenRouter traffic, logs to JSONL."""
...
LOG_FILE = os.environ.get("LLM_PROXY_LOG") or "/home/nebula/or-llm-usage.jsonl"
...
async def handle_request(
    ...
    try:
        ...
        resp = await client.post(
            _forward_url(OPENROUTER_URL, path), headers=headers, content=body_raw
        )
...
async def main() -> None:
    ...
    print(
        f"LLM monitor proxy on http://{addr[0]}:{addr[1]} -> {OPENROUTER_URL}\n"
        f"Log: {LOG_FILE}",
        file=sys.stderr,
    )
    async with server:
